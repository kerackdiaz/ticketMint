...
const Notifications = () => {
  ...
  const lastNotifications = fiterNameNotify?.map(event => {
    const notificationsCopy = [...event.notifications];
    ...
    const dateA = new Date(a.notification.date);
    const dateB = new Date(b.notification.date);
    return  dateA.getTime() - dateB.getTime();
  });
  
  const viewNotifications = () => {
    return lastNotifications ? [...lastNotifications].reverse().map((event, index) => (
      <Link to={`/messages/${event.id}`} key={index}>
        <div className='border-b-2 border-[#6651c3] flex justify-around items-center py-5'>
          ...
          <p className='text-sm dark:text-white text-[#0b0b1c]'>{moment(event.notification.date).fromNow()}</p>
        </div>
      </Link>
