...
function DetailTicket() {
  const tickets = useSelector((state) => state.authReducer.user.ticket)
  const {id} = useParams()
  ...
  return (
    <div className='bg-[#0b0b1c] p-4 flex flex-1 gap-5 flex-col items-center'>
        <h2 className='text-3xl text-white'>Arch Enemy{ticket.name}</h2>
        <CardMyTickets/>
        ...
        <Link to={`/scan/${id}`} className='w-full flex justify-center md:w-1/2 xl:w-1/3'>
