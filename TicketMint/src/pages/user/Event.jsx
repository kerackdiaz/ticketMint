...
function Event() {
  ...
  const getEvents = useMemo(() => {
    ...
    let filterEvent = Object.values(events)
      ...
      .filter(event => new Date(event.date + "T00:00:00Z") >= new Date());
  ...
  const getNextEvent = useMemo(() => {
    ...
    const futureEvents = Object.values(events).filter(event => new Date(event.date + "T00:00:00Z") >= nowDate);
    if (futureEvents.length === 0) {
      return;
    }
    const dateNextEvent = futureEvents.reduce((min, event) => new Date(event.date + "T00:00:00Z") < new Date(min.date + "T00:00:00Z") ? event : min, futureEvents[0]);
    return <CardEvent key={dateNextEvent.name} idProx={"ProxEvent"} date={dateNextEvent.date} image={dateNextEvent.imageURL} name={dateNextEvent.name} id={dateNextEvent.id} price={dateNextEvent.basePrice} />
  }, [events, isEmpty]);
  ...
  return (
    <main className='flex flex-1 flex-col desktop:mt-20 desktop:flex-wrap items-center '>
      <section className='flex flex-col gap-3 justify-around w-screen items-center desktop:self-start desktop:w-[30%]'>
        <h1 className='text-lg md:text-5xl py-1 dark:text-white text-[#0b0b1c] font-bold movil:mt-5 movil:text-2xl laptop:mt-0 desktop:fixed desktop:top-20 desktop:left-5'>Events</h1>
