...
function CardMyTickets({name, date, image, id, location, ticketCount, time}) {
  return (
    <article>
      <div key={id} className=' shadow-lg shadow-[#0b0b1c] dark:shadow-none desktop:bg-no-repeat desktop:bg-cover desktop:w-[430px] rounded-2xl flex flex-col justify-end  w-[343px] h-64'>
      {ticketCount == undefined ? <></>:<div className='w-full flex justify-end'><div className='relative z-20 bg-red-800 translate-x-4 translate-y-24 w-8 h-8 text-xl flex justify-center items-center text-white rounded-full font-bold border-white border'>{ticketCount}</div></div>}
