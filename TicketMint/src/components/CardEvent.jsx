...
function CardEvent({ handleFav, onFav, name, date, image, id, quantity, time, price, location, idProx }) {
    ...
    return (
        ...
        <Link to={`/details/${id}`} >
            <div id={idProx} className={'relative w-80 h-64 bg-[url("' + image + '")] desktop:bg-no-repeat desktop:bg-cover desktop:w-[430px] bg-top bg-local rounded-2xl flex flex-col justify-end items-end'}>
                ...
                <div className='h-28 bg-[#523276] opacity-90 w-full  rounded-b-2xl flex  justify-between px-2 py-1'>
                    ...
                    <div className='flex flex-col items-start opacity-90'>
                        ...
                        <p className='text-sm text-white w-full text-end'>${price}</p>
                    </div>
                </div>
            </div>
        </Link>
